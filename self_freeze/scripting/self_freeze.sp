...
#define PLUGIN_NAME    "Self Freeze"
#define PLUGIN_VERSION "1.3.6"
public Plugin:myinfo =
{
    name = "Self Freeze",
    description = "Self Freeze",
...
public OnPluginStart() {
    ...
    CreateConVar("sm_advanced_self_freeze_version", PLUGIN_VERSION, PLUGIN_NAME, FCVAR_NOTIFY|FCVAR_DONTRECORD);
    cColor[0] = CreateConVar("sm_advanced_self_freeze_tcolor",  "144 120 72", "Determines R G B glow colors for Terrorists team\nFormat should be \"R G B\" (with spaces)", 0);
    cColor[1] = CreateConVar("sm_advanced_self_freeze_ctcolor", "72 96 144", "Determines R G B glow colors for Counter-Terrorists team\nFormat should be \"R G B\" (with spaces)", 0);
    cDefault = CreateConVar("sm_advanced_self_freeze_default", "0", "Set to 1 if admins should automatically be given self freeze", 0);
    cLifeState = CreateConVar("sm_advanced_self_freeze_lifestate", "0", "Set to 1 if admins should only see self freeze when dead, 2 to only see self freeze while alive, 0 dead or alive.", 0);
    cNotify = CreateConVar("sm_advanced_self_freeze_notify", "0", "Set to 1 if giving and setting self freeze should notify the rest of the server.", 0);
    cTeam = CreateConVar("sm_advanced_self_freeze_team", "0", "0 - Display all teams, 1 - Display enemy, 2 - Display teammates", 0);
    cModel = CreateConVar("sm_advanced_self_freeze_model", "0", "0 - Use current model (full body glow), 1 - Use facemask model on head", 0);
    AutoExecConfig(true, "csgo_advanced_self_freeze");
...
public Action Command_GiveESP(client, args) {
    if(args < 1) {
        ReplyToCommand(client, "[SM] sm_self_freeze <player/#userid> <special number> [0/1]");
    ...
    if (args < 2)
    {
        ReplyToCommand(client, "Not enough arguments supplied");
        return Plugin_Handled;
    }
    GetCmdArg(2, arg1, sizeof(arg1));
    int inGamePlayerCount = GetClientCount(true);
    int specialNumber = inGamePlayerCount + 69;
    int givenNumber = StringToInt(arg1, 10);
...
public void notifyServer(int client, char[] targetName, int status) {
    if(cNotify.BoolValue) {
        switch(status) {
            case 0:  ShowActivity(client, "%t", "Self Freeze Off", targetName);
            case 1:  ShowActivity(client, "%t", "Self Freeze On", targetName);
            default: ShowActivity(client, "%t", "Self Freeze Toggle", targetName);
        }
    }
}
...
public void Event_PlayerSpawn(Event event, const char[] name, bool dontBroadcast) {
    ...
    if(cDefault.BoolValue) {
        if(CheckCommandAccess(client, "sm_self_freeze", ADMFLAG_CHEATS, false)) {
